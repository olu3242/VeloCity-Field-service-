...
export function getEnvWithFallback(key: EnvKey, fallback: string): string {
...
export const env = {
  ...
  appUrl: getEnvWithFallback("NEXT_PUBLIC_APP_URL", "http://localhost:3003"),
  apiBaseUrl: getEnvWithFallback("NEXT_PUBLIC_APP_URL", "http://localhost:3003"),
  supabase: {
    url: getEnv("NEXT_PUBLIC_SUPABASE_URL"),
    anonKey: getEnv("NEXT_PUBLIC_SUPABASE_ANON_KEY"),
    serviceRoleKey: getEnv("SUPABASE_SERVICE_ROLE_KEY"),
    enabled: hasEnv("NEXT_PUBLIC_SUPABASE_URL") && hasEnv("NEXT_PUBLIC_SUPABASE_ANON_KEY"),
    adminEnabled: hasEnv("NEXT_PUBLIC_SUPABASE_URL") && hasEnv("SUPABASE_SERVICE_ROLE_KEY"),
  },
  stripe: {
    publishableKey: getEnv("NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY"),
    secretKey: getEnv("STRIPE_SECRET_KEY"),
    webhookSecret: getEnv("STRIPE_WEBHOOK_SECRET"),
    enabled: hasEnv("NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY") && hasEnv("STRIPE_SECRET_KEY"),
  },
  cronSecret: getEnv("CRON_SECRET"),
  mapsKey: getEnv("NEXT_PUBLIC_GOOGLE_MAPS_API_KEY"),
  ai: {
    anthropicKey: getEnv("ANTHROPIC_API_KEY"),
    enabled: hasEnv("ANTHROPIC_API_KEY"),
  },
  features: {
    mockSupabaseInDevelopment: process.env.NODE_ENV !== "production" && !hasEnv("NEXT_PUBLIC_SUPABASE_URL"),
    runtimeWarnings: process.env.NODE_ENV !== "production",
  },
} as const;

export function getRuntimeWarning(message: string) {
  if (env.features.runtimeWarnings) {
    return `[VeloCity config] ${message}`;
  }
  return null;
}
