...
export function calculatePrice(input: PricingInput): PricingResult {
  const basePrice = getBasePrice(input.category);
  const pricingMode = getPricingMode(input.category, input.urgency === "emergency");
  const laborAdjustment = Math.max(0, Math.round((input.laborHours ?? 1) - 1) * 6500);
  const materialsEstimate = Math.max(0, input.materialsEstimateCents ?? 0);
  const urgencyAdjustment = calculateUrgencyAdjustment(basePrice, input.urgency);
  const locationAdjustment = calculateLocationAdjustment(basePrice, input.state, input.zip);
  const complexityAdjustment = calculateComplexityAdjustment(basePrice, input.complexity);
  const surgeAdjustment = calculateSurgeAdjustment(basePrice, input.urgency);
